...
import { assignReservationToRoom, createClient, createContactLog, createRoomGroup, deleteClientWithReservations, deleteReservation, getSession, listClients, listContactLogs, listDepartures, listPayments, listReservations, listRoomGroups, markPaymentAsPaid, replacePendingPaymentPlan, signOut, supabaseConfigured, updateClient, type ClientRecord, type ContactLogRecord, type DepartureRecord, type PaymentRecord, type ReservationRecord, type RoomGroupRecord } from "../../lib/supabase-browser";
...
function ContractDocumentsModule({ departure, reservations, payments }: { departure: DepartureRecord; reservations: ReservationRecord[]; payments: PaymentRecord[] }) {
  const contracts: Record<string, { activity: string; included: string[]; excluded: string[]; risks: string }> = {
    "andes-essencial": { activity: "trekking de montanha, com deslocamentos em altitude e atividades ao ar livre na região de Mendoza", included: ["7 diárias em pousadas ou hotéis padrão turístico ou superior, com café da manhã", "1 almoço harmonizado e 1 jantar harmonizado", "ingressos aos atrativos previstos no programa", "transportes mencionados no programa", "camiseta Ekonova Adventure e acompanhamento de guia Cadastur"], excluded: ["refeições, bebidas e deslocamentos não mencionados no programa", "hospedagem não mencionada no roteiro", "seguro viagem, despesas pessoais e qualquer item não citado como incluído"], risks: "altitude, clima variável, trilhas e terrenos irregulares, deslocamentos em áreas remotas e esforço físico compatível com a atividade" },
    "trekking-caminhos-do-ouro": { activity: "trekking pelas montanhas e caminhos históricos de Minas Gerais", included: ["7 noites em pousadas com quartos duplos", "cafés da manhã, 2 almoços e jantares descritos no programa", "5 dias de trekking", "entradas nos atrativos mencionados", "assistência, logística e guia Cadastur durante a jornada"], excluded: ["refeições, bebidas e serviços não expressamente incluídos", "despesas decorrentes de acidentes atribuíveis ao participante ou a terceiros", "hospedagem single, sujeita ao suplemento contratado"], risks: "trilhas, subidas e descidas, terreno irregular, variações climáticas, fadiga e distância de centros de atendimento" },
    "trekking-atacama-essencia-2027": { activity: "trekking e experiências culturais no deserto do Atacama", included: ["8 noites no Hotel Tocona, com café da manhã", "recepção no aeroporto e transfers Calama - San Pedro - Calama", "transportes e ingressos previstos no roteiro", "turismo astronômico, refeições indicadas no programa e acompanhamento de guias", "coordenação de reservas, horários e atividades"], excluded: ["refeições, bebidas e deslocamentos não mencionados", "hospedagem não prevista no roteiro", "seguro viagem, despesas pessoais e hospedagem single, sujeita ao suplemento contratado"], risks: "altitude, clima árido, grande amplitude térmica, terreno irregular, exposição solar e distância de centros de atendimento" },
    "biketour-caminhos-do-ouro": { activity: "biketour por estradas rurais, trilhas e caminhos históricos de Minas Gerais", included: ["6 noites em pousadas com quartos duplos", "cafés da manhã e 2 jantares indicados", "5 dias de pedal e tracks GPX", "veículo de apoio para bagagens quando necessário", "rastreador SPOT e acompanhamento de guia Cadastur"], excluded: ["refeições, bebidas e serviços não expressamente incluídos", "bicicleta, capacete, equipamentos pessoais e manutenção", "despesas decorrentes de acidentes atribuíveis ao participante ou a terceiros", "hospedagem single, sujeita ao suplemento contratado"], risks: "tráfego, quedas, terreno irregular, falhas mecânicas, variações climáticas e esforço físico prolongado" },
  };
  ...
  function generate(reservation: ReservationRecord) {
    ...
    const popup = window.open("", "_blank"); if (!popup) return; popup.opener = null; popup.document.write(html); popup.document.close();
  }
