...
function FinanceModule({ payments, departures, onAction, onUpdated }: { payments: PaymentRecord[]; departures: DepartureRecord[]; onAction: (message: string) => void; onUpdated: () => Promise<void> }) {
  ...
  const totals = (itemCurrency: "USD" | "BRL") => { const list = visible.filter((item) => currency(item) === itemCurrency); const planned = list.reduce((sum, item) => sum + item.amount_cents, 0); const received = list.filter((item) => item.status === "pago").reduce((sum, item) => sum + item.amount_cents, 0); return { planned, received, open: planned - received }; };
  const currencies = (["BRL", "USD"] as const).filter((itemCurrency) => visible.some((item) => currency(item) === itemCurrency));
  const open = visible.filter((item) => item.status !== "pago").sort((a, b) => a.due_on.localeCompare(b.due_on));
  ...
  return <section className="module-page"><p className="module-eyebrow">Financeiro operacional</p><h1>Pagamentos por saída</h1><p className="module-summary">Acompanhe parcelas reais sem misturar moedas ou roteiros. A confirmação de pagamento fica registrada com a data de hoje.</p><section className="finance-filter"><label>Saída<select value={departureId} onChange={(event) => setDepartureId(event.target.value)}><option value="all">Todas as saídas</option>{departures.map((departure) => <option key={departure.id} value={departure.id}>{departure.trips?.title} · {departure.currency}</option>)}</select></label><span>{open.length} parcela(s) em aberto</span></section>{currencies.length ? <div className="finance-currency-grid">{currencies.map((itemCurrency) => { const total = totals(itemCurrency); return <article key={itemCurrency}><small>{itemCurrency}</small><span>Previsto</span><strong>{money(total.planned, itemCurrency)}</strong><div><b>Recebido {money(total.received, itemCurrency)}</b><em>Em aberto {money(total.open, itemCurrency)}</em></div></article>; })}</div> : <section className="empty-section"><h2>Sem lançamentos financeiros</h2><p>As parcelas serão criadas automaticamente quando uma inscrição for concluída.</p></section>}<section className="finance-table">{visible.map((payment) => { const itemCurrency = currency(payment); const due = new Date(`${payment.due_on}T12:00:00`).toLocaleDateString("pt-BR"); return <article key={payment.id}><span className={payment.status === "pago" ? "paid" : "pending"}>{payment.status === "pago" ? "✓" : "!"}</span><div><strong>{payment.reservations?.clients?.full_name ?? "Cliente"}</strong><small>{payment.reservations?.departures?.trips?.title ?? "Saída"} · {payment.reference ?? "Parcela"} · vence em {due}</small></div><b>{money(payment.amount_cents, itemCurrency)}</b>{payment.status === "pago" ? <em>Pago em {payment.paid_on ? new Date(`${payment.paid_on}T12:00:00`).toLocaleDateString("pt-BR") : "data não informada"}</em> : <button disabled={savingId === payment.id} onClick={() => markPaid(payment)}>{savingId === payment.id ? "Confirmando…" : "Confirmar pagamento"}</button>}</article>; })}</section></section>;
}
