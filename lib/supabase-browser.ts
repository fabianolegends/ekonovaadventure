...
export async function getMyProfile() {
  ...
  const response = await fetch(endpoint(`/rest/v1/team_members?id=eq.${encodeURIComponent(session.user.id)}&select=id,full_name,role,active,avatar_url`), {
...
export async function updateMyProfile(input: Pick<TeamMemberRecord, "full_name" | "avatar_url">) {
  const session = await getValidSession();
  if (!session) throw new Error("Sua sessão expirou. Entre novamente.");
  const response = await fetch(endpoint(`/rest/v1/team_members?id=eq.${encodeURIComponent(session.user.id)}`), {
    method: "PATCH",
    headers: { apikey: key!, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Prefer: "return=representation" },
